...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://haikal:password@localhost/db_pets'
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
# postgresql://postgres:mysecretpassword@localhost
db = SQLAlchemy(app)
CORS(app)
print("DATABASE CONNECTED")

# Define class Pet Model for create table as Model
class Pet(db.Model):
    """
    Class Pet Model is an instance of table pets in our database.
    """
    __tablename__ = 'pets'
...
def update_pet(pet_id):
    ...
    if pet is None:
        return jsonify({"success": True, "response": "Pet not found"})
    else:
        pet.pet_age = pet_age
...
def delete_pet(pet_id):
    ...
    if pet is None:
        ...
        db.session.commit()
        return jsonify({"success": True, "response": f"Pet ID {pet_id} DELETED"})

if __name__ == '__main__':
    print("CREATE DATABASE ASSET")
    db.create_all()
